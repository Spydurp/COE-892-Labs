...
def run(roverID: int, mapID: str):
    ...
    while mines > 0:
        ...
        if cmd == 'R':
            ...
            if map_arr[y][x] != 0:
                ...
                map_arr[y][x] = 0
                mines -= 1
                send_pin = stub.sharePin(groundControl_pb2.pinSend(pinVal=str(pin), id=int(roverID), serial=str(serial)))

                if send_pin.success:
                    print("Pin Acknowledge: " + send_pin.message)
        ...
        if mine is True:
            print("Boom")
            stub.reportStatus(groundControl_pb2.status(success=False, id=int(roverID)))
            break
    ...
    if mines == 0:
        stub.reportStatus(groundControl_pb2.status(success=True, id=int(roverID)))
